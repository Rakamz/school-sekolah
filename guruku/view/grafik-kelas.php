            ...
            <script type="text/javascript">
                ...
                function drawChart() {
                    ...
                    var data = google.visualization.arrayToDataTable([
                        ...
                        <?php
                        require '../function/config.php';
                        $query = $conn->query("SELECT * FROM kelas GROUP by nama_kelas");
                        
                        // Membuat array asosiatif untuk mengelompokkan data berdasarkan nama nama_kelas
                        $dataByNama = array();
                        foreach ($query as $key => $value) {
                            $nama_kelas = $value['nama_kelas'];
                            $nama = trim(preg_replace('/[0-9]+/', '', $nama_kelas));
                            if (!isset($dataByNama[$nama])) {
                                $dataByNama[$nama] = 0;
                            }
                            $dataByNama[$nama] += mysqli_num_rows($conn->query("SELECT * FROM kelas WHERE nama_kelas = '" . $value['nama_kelas'] . "'"));
                        }

                        // Mengisi data ke dalam array yang akan digunakan untuk menggambar grafik
                        foreach ($dataByNama as $nama => $total) {
                            echo ",['" . $nama . "', " . $total . "]";
                        }
                        ?>
                    ]);
